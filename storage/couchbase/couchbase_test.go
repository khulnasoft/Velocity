...
const (
	...
	couchbaseBucket      string = "velocity_storage"
)
...
func TestSetAndGet_GetShouldReturn_SetValueWithoutError(t *testing.T) {
	...
	err = testStore.Set("test", []byte("velocity_test_value"), 0)
	require.NoError(t, err)

	val, err := testStore.Get("test")

	require.NoError(t, err)
	require.Equal(t, val, []byte("velocity_test_value"))
}
...
func TestSetAndGet_GetShouldReturnNil_WhenTTLExpired(t *testing.T) {
	...
	err = testStore.Set("test", []byte("velocity_test_value"), 3*time.Second)
	require.NoError(t, err)
