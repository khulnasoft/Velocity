...
func Test_Parser_Request_URL(t *testing.T) {
	...
	t.Run("the path param from request", func(t *testing.T) {
		...
		err := parserRequestURL(client, req)
		require.NoError(t, err)
		require.Equal(t, "http://example.com/api/5/velocity/%7Bkey%7D", req.RawRequest.URI().String())
	})
	...
	t.Run("the path param from request and client", func(t *testing.T) {
		...
		err := parserRequestURL(client, req)
		require.NoError(t, err)
		require.Equal(t, "http://example.com/api/12/velocity/val", req.RawRequest.URI().String())
	})
...
func Test_Parser_Request_Header(t *testing.T) {
	...
	t.Run("client header should be set", func(t *testing.T) {
		...
		client := New().
			SetHeaders(map[string]string{
				velocity.HeaderContentType: "application/json",
			})
	...
	t.Run("request header should be set", func(t *testing.T) {
		...
		req := AcquireRequest().
			SetHeaders(map[string]string{
				velocity.HeaderContentType: "application/json, utf-8",
			})
	...
	t.Run("request header should override client header", func(t *testing.T) {
		t.Parallel()
		client := New().
			SetHeader(velocity.HeaderContentType, "application/xml")

		req := AcquireRequest().
			SetHeader(velocity.HeaderContentType, "application/json, utf-8")
	...
	t.Run("auto set file header", func(t *testing.T) {
		...
		err := parserRequestHeader(client, req)
		require.NoError(t, err)
		require.Contains(t, string(req.RawRequest.Header.MultipartFormBoundary()), "--VelocityFormBoundary")
	...
	t.Run("ua should have default value", func(t *testing.T) {
		...
		err := parserRequestHeader(client, req)
		require.NoError(t, err)
		require.Equal(t, []byte("velocity"), req.RawRequest.Header.UserAgent())
	})
...
func Test_Parser_Request_Body(t *testing.T) {
	...
	t.Run("file body", func(t *testing.T) {
		...
		err := parserRequestBody(client, req)
		require.NoError(t, err)
		require.Contains(t, string(req.RawRequest.Body()), "----VelocityFormBoundary")
...
func Test_Client_Logger_Debug(t *testing.T) {
	...
	go func() {
		assert.NoError(t, app.Listen(":0", velocity.ListenConfig{
