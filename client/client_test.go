...
func startTestServerWithPort(t *testing.T, beforeStarting func(app *velocity.App)) (*velocity.App, string) {
	...
	go func() {
		err := app.Listen(":0", velocity.ListenConfig{
...
func Test_Get(t *testing.T) {
	...
	setupApp := func() (*velocity.App, string) {
		app, addr := startTestServerWithPort(t, func(app *velocity.App) {
			app.Get("/", func(c velocity.Ctx) error {
				return c.SendString(c.Hostname())
			})
		})
...
func Test_Head(t *testing.T) {
	...
	t.Run("global head function", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, "7", resp.Header(velocity.HeaderContentLength))
...
func Test_Options(t *testing.T) {
	...
	setupApp := func() (*velocity.App, string) {
		app, addr := startTestServerWithPort(t, func(app *velocity.App) {
			app.Options("/", func(c velocity.Ctx) error {
				c.Set(velocity.HeaderAllow, "GET, POST, PUT, DELETE, PATCH")
				return c.Status(velocity.StatusNoContent).SendString("")
			})
		})
	...
	t.Run("global options function", func(t *testing.T) {
		...
		for i := 0; i < 5; i++ {
			...
			require.NoError(t, err)
			require.Equal(t, "GET, POST, PUT, DELETE, PATCH", resp.Header(velocity.HeaderAllow))
			require.Equal(t, velocity.StatusNoContent, resp.StatusCode())
			require.Equal(t, "", resp.String())
		}
	})
...
func Test_Client_TLS(t *testing.T) {
	...
	go func() {
		assert.NoError(t, app.Listener(ln, velocity.ListenConfig{
			DisableStartupMessage: true,
		}))
	}()
	time.Sleep(1 * time.Second)
...
func Test_Client_SetProxyURL(t *testing.T) {
	...
	proxyServer := velocity.New()

	proxyServer.Use("*", func(c velocity.Ctx) error {
	...
	addrChan := make(chan string)
	go func() {
		assert.NoError(t, proxyServer.Listen(":0", velocity.ListenConfig{
			DisableStartupMessage: true,
			ListenerAddrFunc: func(addr net.Addr) {
				addrChan <- addr.String()
			},
