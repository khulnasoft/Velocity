...
func Test_Exec_Func(t *testing.T) {
	...
	app.Get("/normal", func(c velocity.Ctx) error {
		return c.SendString(c.Hostname())
	})

	app.Get("/return-error", func(_ velocity.Ctx) error {
		return errors.New("the request is error")
	})

	app.Get("/hang-up", func(c velocity.Ctx) error {
		time.Sleep(time.Second)
		return c.SendString(c.Hostname() + " hang up")
	})

	go func() {
		assert.NoError(t, app.Listener(ln, velocity.ListenConfig{DisableStartupMessage: true}))
	}()
