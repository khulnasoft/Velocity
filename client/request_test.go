...
func Test_Request_Header(t *testing.T) {
	...
	t.Run("add header", func(t *testing.T) {
		t.Parallel()
		req := AcquireRequest()
		req.AddHeader("foo", "bar").AddHeader("foo", "velocity")

		res := req.Header("foo")
		require.Len(t, res, 2)
		require.Equal(t, "bar", res[0])
		require.Equal(t, "velocity", res[1])
	})

	t.Run("set header", func(t *testing.T) {
		t.Parallel()
		req := AcquireRequest()
		req.AddHeader("foo", "bar").SetHeader("foo", "velocity")

		res := req.Header("foo")
		require.Len(t, res, 1)
		require.Equal(t, "velocity", res[0])
	})
...
func Test_Request_Headers(t *testing.T) {
	...
	require.Contains(t, headers["Foo"], "velocity")
...
func Test_Request_QueryParam(t *testing.T) {
	...
	t.Run("add param", func(t *testing.T) {
		t.Parallel()
		req := AcquireRequest()
		req.AddParam("foo", "bar").AddParam("foo", "velocity")
	...
	t.Run("set param", func(t *testing.T) {
		t.Parallel()
		req := AcquireRequest()
		req.AddParam("foo", "bar").SetParam("foo", "velocity")
...
func Test_Request_Params(t *testing.T) {
	...
	require.Contains(t, pathParams["foo"], "velocity")
...
func Test_Request_UA(t *testing.T) {
	...
	req := AcquireRequest().SetUserAgent("velocity")
	require.Equal(t, "velocity", req.UserAgent())
...
func Test_Request_FormData(t *testing.T) {
	...
	t.Run("add form data", func(t *testing.T) {
		...
		req.AddFormData("foo", "bar").AddFormData("foo", "velocity")
	...
	t.Run("set param", func(t *testing.T) {
		...
		req := AcquireRequest()
		defer ReleaseRequest(req)
		req.AddFormData("foo", "bar").SetFormData("foo", "velocity")
	...
	t.Run("add params", func(t *testing.T) {
		...
		require.Contains(t, res, "velocity")
...
func Test_Request_Post(t *testing.T) {
	...
	app.Post("/", func(c velocity.Ctx) error {
		return c.Status(velocity.StatusCreated).
	...
	for i := 0; i < 5; i++ {
		...
		require.NoError(t, err)
		require.Equal(t, velocity.StatusCreated, resp.StatusCode())
...
func Test_Request_Head(t *testing.T) {
	...
	app, ln, start := createHelperServer(t)
	app.Head("/", func(c velocity.Ctx) error {
		return c.SendString(c.Hostname())
	})
...
func Test_Request_Put(t *testing.T) {
	...
	app, ln, start := createHelperServer(t)
	app.Put("/", func(c velocity.Ctx) error {
...
func Test_Request_Delete(t *testing.T) {
	...
	app.Delete("/", func(c velocity.Ctx) error {
		return c.Status(velocity.StatusNoContent).
			SendString("deleted")
	})
	...
	for i := 0; i < 5; i++ {
		...
		require.NoError(t, err)
		require.Equal(t, velocity.StatusNoContent, resp.StatusCode())
		require.Equal(t, "", resp.String())
...
func Test_Request_Options(t *testing.T) {
	...
	app.Options("/", func(c velocity.Ctx) error {
		return c.Status(velocity.StatusOK).
			SendString("options")
	})
...
func Test_Request_Patch(t *testing.T) {
	...
	app.Patch("/", func(c velocity.Ctx) error {
		return c.SendString(c.FormValue("foo"))
	})
...
func Test_Request_Body_With_Server(t *testing.T) {
	...
	t.Run("formdata", func(t *testing.T) {
		...
		testRequest(t,
			func(c velocity.Ctx) error {
				require.Equal(t, velocity.MIMEApplicationForm, string(c.Request().Header.ContentType()))
				return c.Send([]byte("foo=" + c.FormValue("foo") + "&bar=" + c.FormValue("bar") + "&velocity=" + c.FormValue("velocity")))
			},
			func(agent *Request) {
	...
	t.Run("multipart form", func(t *testing.T) {
		...
		app, ln, start := createHelperServer(t)
		app.Post("/", func(c velocity.Ctx) error {
			require.Equal(t, "multipart/form-data; boundary=myBoundary", c.Get(velocity.HeaderContentType))
	...
	t.Run("multipart random boundary", func(t *testing.T) {
		...
		app.Post("/", func(c velocity.Ctx) error {
			...
			require.True(t, reg.MatchString(c.Get(velocity.HeaderContentType)))
...
func Test_Request_MaxRedirects(t *testing.T) {
	...
	go func() { assert.NoError(t, app.Listener(ln, velocity.ListenConfig{DisableStartupMessage: true})) }()
