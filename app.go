...
// App denotes the Velocity application.
type App struct {
	// Ctx pool
	pool sync.Pool
	// Fasthttp server
...
type Config struct { //nolint:govet // Aligning the struct fields is not necessary. betteralign:ignore
	...
	// PassLocalsToViews Enables passing of the locals set on a velocity.Ctx to the template engine
	//
	// Default: false
	...
	//
	// Default: map[string]string{"gzip": ".velocity.gz", "br": ".velocity.br", "zstd": ".velocity.zst"}
	...
	// ErrorHandler is executed when an error is returned from velocity.Handler.
	//
	...
	// When set by an external client of Velocity it will use the provided implementation of a
	// JSONMarshal
	//
	...
	// XMLEncoder set by an external client of Velocity it will use the provided implementation of a
	// XMLMarshal
	//
	...
	// XMLDecoder set by an external client of Velocity it will use the provided implementation of a
	// XMLUnmarshal
	//
	...
	// If you enable TrustProxy and do not provide a TrustProxyConfig, Velocity will skip
	...
	// Velocity doesn't have default validator, so it'll skip validator step if you don't use any validator.
	//
	// Default: nil
...
// New creates a new Velocity named instance.
//
//	app := velocity.New()
//
...
// compose them as a single service using Use. The velocity's error handler and
// any of the velocity's sub apps are added to the application's error handlers
...
//		     return c.Next()
//		})
//		app.Use("/api", handler, func(c velocity.Ctx) error {
//		     return c.Next()
//		})
//	 	subApp := velocity.New()
...
// sub velocitys by their prefixes and if it finds a match, it uses that
...
// user for the fasthttp server configuration. It maps a set of fasthttp errors to velocity
