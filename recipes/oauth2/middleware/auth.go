...
func OAUTHRedirect(ctx *velocity.Ctx) error {
	...
	code := ctx.Query("code", "")
	if len(code) < 1 {
		return ctx.SendStatus(velocity.StatusBadRequest)
	}

	// Next, lets for the HTTP request to call the github oauth enpoint	to get our access token

	a := velocity.AcquireAgent()
	req := a.Request()
	req.Header.SetMethod(velocity.MethodPost)
	...
	if retCode, retBody, errs = a.Struct(&t); len(errs) > 0 {
		...
		return ctx.SendStatus(velocity.StatusInternalServerError)
	}
	...
	if sess, err = models.MySessionStore.Get(ctx); err == nil {
		...
		//		return ctx.Redirect("/welcome.html?access_token="+t.AccessToken, velocity.StatusFound)
		return ctx.Redirect("/welcome.html", velocity.StatusFound)
	}

	models.SYSLOG.Tracef("redirecting to /")
	return ctx.Redirect("/", velocity.StatusTemporaryRedirect)
}

// OAUTHProtected processes access attempts; if the session stored token is NULL then it sends to start page
func OAUTHProtected(c *velocity.Ctx) error {
	...
	if tk == nil {
		...
		return c.Redirect("/index.html", velocity.StatusTemporaryRedirect)
	}
