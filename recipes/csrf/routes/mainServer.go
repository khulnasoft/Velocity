...
func init() {
	sessionStore.RegisterType(velocity.Map{})
...
func RegisterRoutes(app *velocity.App) {
	...
	app.Get("/", requireLogin, csrfProtection, func(c *velocity.Ctx) error {
		currSession, err := sessionStore.Get(c)
		if err != nil {
			return err
		}
		sessionUser := currSession.Get("User").(velocity.Map)
		...
		if sessionUser["Name"] == "" {
			return c.Status(velocity.StatusBadRequest).SendString("User is empty")
		}
		username := sessionUser["Name"].(string)

		return c.Render("views/home", velocity.Map{
			"username":  username,
	...
	app.Get("/login", func(c *velocity.Ctx) error {
		return c.Render("views/login", velocity.Map{})
	})
	...
	app.Post("/login", func(c *velocity.Ctx) error {
		...
		if user.Username == "" {
			return c.Status(velocity.StatusBadRequest).SendString("Username is required.")
		}

		if user.Password == "" {
			return c.Status(velocity.StatusBadRequest).SendString("Password is required.")
		}

		if !findUser(validLogins, user) {
			return c.Status(velocity.StatusBadRequest).SendString("Invalid username or password.")
		}
		...
		currSession.Set("User", velocity.Map{"Name": user.Username})
	...
	app.Post("/transfer", requireLogin, csrfProtection, func(c *velocity.Ctx) error {
		...
		if transfer.To == "" {
			return c.Status(velocity.StatusBadRequest).SendString("\"To account\" is required.")
		}
		currSession, err := sessionStore.Get(c)
		if err != nil {
			return err
		...
		if sessionUser["Name"] == "" {
			return c.Status(velocity.StatusBadRequest).SendString("\"From account\" is required.")
		}
		username := sessionUser["Name"].(string)

		if _, ok := accounts[transfer.To]; !ok {
			return c.Status(velocity.StatusBadRequest).SendString("Cannot transfer funds to non-existent account (\"" + transfer.To + "\").")
		}

		if _, ok := accounts[username]; !ok {
			return c.Status(velocity.StatusBadRequest).SendString("Cannot transfer funds from non-existent account (\"" + transfer.To + "\").")
		}

		if transfer.Amount == 0 {
			return c.Status(velocity.StatusBadRequest).SendString("\"Amount\" is required.")
		}
...
func requireLogin(c *velocity.Ctx) error {
	currSession, err := sessionStore.Get(c)
	if err != nil {
		return err
	}
