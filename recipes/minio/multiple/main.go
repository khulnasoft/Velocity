// ⚡️ Velocity: A Go-based web framework inspired by Express
...
import (
	...
	"go.khulnasoft.com/velocity/storage/minio"
...
func main() {
	...
	store := minio.New(minio.Config{
		Endpoint: getEnv("VELOCITY_MINIO_ENDPOINT", "localhost:9000"),
		Secure:   getEnv("VELOCITY_MINIO_USE_SSL", "false") == "true",
		Bucket:   getEnv("VELOCITY_MINIO_BUCKET", "velocity-bucket"),
		Region:   getEnv("VELOCITY_MINIO_REGION", "us-east-1"),
		Credentials: minio.Credentials{
			AccessKeyID:     getEnv("VELOCITY_MINIO_ACCESS_KEY", "minioadmin"),
			SecretAccessKey: getEnv("VELOCITY_MINIO_SECRET_KEY", "minioadmin"),
		},
	})
	...
	app.Post("/upload", func(c *velocity.Ctx) error {
		...
		if err != nil {
			return c.Status(http.StatusBadRequest).JSON(velocity.Map{
		...
		var uploadedFiles []velocity.Map // List to store information about successfully uploaded files
		var failedFiles []velocity.Map   // List to store information about files that failed to upload
		...
		for _, filePart := range files {
			...
			if filePart.Size > maxFileSize {
				failedFiles = append(failedFiles, velocity.Map{
			...
			uploadedFiles = append(uploadedFiles, velocity.Map{
				"filename": filePart.Filename,
				"url":      fileURL,
			})
		}
		...
		return c.Status(http.StatusOK).JSON(velocity.Map{
	...
	app.Get("/file/:filename", func(c *velocity.Ctx) error {
		...
		if err != nil {
			return c.Status(http.StatusNotFound).JSON(velocity.Map{
		...
		if err != nil {
			return c.Status(http.StatusInternalServerError).JSON(velocity.Map{
				"message": "error retrieving file",
			})
		}

		// Set HTTP headers to indicate a file download
		c.Set(velocity.HeaderContentDisposition, fmt.Sprintf("attachment; filename=%s", filename))
		c.Set(velocity.HeaderContentType, velocity.MIMEOctetStream)

		// Stream the file contents to the client
		return c.SendStream(object)
	})

	// Start the Velocity server on port 3000
