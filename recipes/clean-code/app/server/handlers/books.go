...
func GetBooks(service services.BooksService) velocity.Handler {
	return func(c *velocity.Ctx) error {
		books, err := service.GetBooks(c.UserContext())
		if err != nil {
			slog.Error("GetBooks failed", "error", err)
			return sendError(c, velocity.StatusInternalServerError, "internal error")
		}
...
func AddBook(service services.BooksService) velocity.Handler {
	return func(c *velocity.Ctx) error {
		var book domain.Book
		if err := c.BodyParser(&book); err != nil {
			slog.Warn("AddBook request parsing failed", "error", err)
			return sendError(c, velocity.StatusBadRequest, "invalid request")
		}
...
func sendError(c *velocity.Ctx, code int, message string) error {
