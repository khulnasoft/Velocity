...
func main() {
	...
	// CSRF Error handler
	csrfErrorHandler := func(c *velocity.Ctx, err error) error {
		...
		default:
			...
			return c.Status(velocity.StatusForbidden).SendString("403 Forbidden")
		}
	}
	...
	app.Get("/login", csrfMiddleware, func(c *velocity.Ctx) error {
		csrfToken, ok := c.Locals("csrf").(string)
		if !ok {
			return c.SendStatus(velocity.StatusInternalServerError)
		}

		return c.Render("login", velocity.Map{
			"Title": "Login",
			"csrf":  csrfToken,
		})
	})
	...
	app.Post("/login", csrfMiddleware, func(c *velocity.Ctx) error {
	...
	app.Get("/logout", func(c *velocity.Ctx) error {
	...
	app.Get("/protected", csrfMiddleware, func(c *velocity.Ctx) error {
	...
	app.Post("/protected", csrfMiddleware, func(c *velocity.Ctx) error {
		// Check if the user is logged in
		session, err := store.Get(c)
		if err != nil {
			return c.SendStatus(velocity.StatusInternalServerError)
