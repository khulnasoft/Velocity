...
func (h *greeterRouteHandler) Register(app *velocity.App) {
	app.Get("/say-hello", h.HandleSayHelloRequest)
}

// HandleSayHelloRequest Handles "GET /say-hello" requests.
func (h *greeterRouteHandler) HandleSayHelloRequest(ctx *velocity.Ctx) error {
	name := ctx.Query("name")
	...
	return ctx.Status(velocity.StatusOK).Send([]byte(msg))
}
