...
func main() {
	...
	db, err := sql.Open("sqlite3", "./velocity.db")
	if err != nil {
		log.Fatal(err)
	}
	defer db.Close()
	...
	// Handle login API
	app.Post("/api/login", func(c *velocity.Ctx) error {
		req := struct {
			UID string `json:"uid"`
		}{}
	...
	// Handle logout API
	app.Post("/api/logout", func(c *velocity.Ctx) error {
		req := struct {
			SID string `json:"sid"`
		}{}
		if err := c.BodyParser(&req); err != nil {
	...
	// Handle account API
	app.Get("/api/account", func(c *velocity.Ctx) error {
		// Get current session
		s, _ := store.Get(c)
