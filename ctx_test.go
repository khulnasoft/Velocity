...
func Test_Ctx_SendFile_ContentType(t *testing.T) {
	...
	// 1) simple case
	c := app.AcquireCtx(&fasthttp.RequestCtx{})
	err := c.SendFile("./.github/testdata/fs/img/velocity.png")
	// check expectation
	require.NoError(t, err)
	require.Equal(t, StatusOK, c.Response().StatusCode())
	require.Equal(t, "image/png", string(c.Response().Header.Peek(HeaderContentType)))
	...
	c = app.AcquireCtx(&fasthttp.RequestCtx{})
	err = c.SendFile("./.github/testdata/fs/img/velocitypng.jpeg")
	// check expectation
	require.NoError(t, err)
	require.Equal(t, StatusOK, c.Response().StatusCode())
	...
	c = app.AcquireCtx(&fasthttp.RequestCtx{})
	err = c.SendFile("./.github/testdata/fs/img/velocitypng.notvalidext")
	// check expectation
	require.NoError(t, err)
	require.Equal(t, StatusOK, c.Response().StatusCode())
	require.Equal(t, "image/png", string(c.Response().Header.Peek(HeaderContentType)))
	...
	c = app.AcquireCtx(&fasthttp.RequestCtx{})
	err = c.SendFile("./.github/testdata/fs/img/velocitypng")
	// check expectation
	require.NoError(t, err)
	require.Equal(t, StatusOK, c.Response().StatusCode())
	require.Equal(t, "image/png", string(c.Response().Header.Peek(HeaderContentType)))
...
func Test_Ctx_RenderWithOverwrittenViewBind(t *testing.T) {
	...
	require.Equal(t, "<h1>Hello from Velocity!</h1>", string(c.Response().Body()))
}
...
func Benchmark_Ctx_Get_Location_From_Route(b *testing.B) {
	...
	var location string
	for n := 0; n < b.N; n++ {
		location, err = c.getLocationFromRoute(app.GetRoute("User"), Map{"name": "velocity"})
	}

	require.Equal(b, "/user/velocity", location)
	require.NoError(b, err)
}
...
func Test_Ctx_Get_Location_From_Route_name(t *testing.T) {
	...
	t.Run("case insensitive", func(t *testing.T) {
		...
		location, err := c.GetRouteURL("User", Map{"name": "velocity"})
		require.NoError(t, err)
		require.Equal(t, "/user/velocity", location)
...
func Test_Ctx_Render_Go_Template(t *testing.T) {
	t.Parallel()
	file, err := os.CreateTemp(os.TempDir(), "velocity")
	require.NoError(t, err)
	defer func() {
		err := os.Remove(file.Name())
		require.NoError(t, err)
...
func Test_GenericParseTypeString(t *testing.T) {
	...
	tests := []string{"john", "doe", "hello", "velocity"}
