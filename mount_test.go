...
func Test_App_UseParentErrorHandler(t *testing.T) {
	...
	velocity := New()
	velocity.Get("/", func(_ Ctx) error {
		return errors.New("something happened")
	})

	app.Use("/api", velocity)
...
func Test_App_UseMountedErrorHandlerRootLevel(t *testing.T) {
	...
	velocity.Get("/api", func(_ Ctx) error {
		return errors.New("something happened")
	})
...
func Test_App_UseMountedErrorHandlerForBestPrefixMatch(t *testing.T) {
	...
	tsf := func(c Ctx, _ error) error {
		return c.Status(200).SendString("hi, i'm a custom sub velocity error 2")
	}
	tripleSubVelocity := New(Config{
		ErrorHandler: tsf,
	})
	tripleSubVelocity.Get("/", func(_ Ctx) error {
		return errors.New("something happened")
	})

	sf := func(c Ctx, _ error) error {
		return c.Status(200).SendString("hi, i'm a custom sub velocity error")
	}
	subvelocity := New(Config{
		ErrorHandler: sf,
	})
	subvelocity.Get("/", func(_ Ctx) error {
		return errors.New("something happened")
	})
	subvelocity.Use("/third", tripleSubVelocity)
	...
	require.Equal(t, "hi, i'm a custom sub velocity error", string(b), "Response body")
	...
	require.NoError(t, err, "iotuil.ReadAll()")
	require.Equal(t, "hi, i'm a custom sub velocity error 2", string(b), "Third velocity Response body")
}
