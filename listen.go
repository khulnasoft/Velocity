...
// Figlet text to show Velocity ASCII art on startup message
var figletVelocityText = `
    _______ __             
   / ____(_) /_  ___  _____
  / /_  / / __ \/ _ \/ ___/
 / __/ / / /_/ /  __/ /    
...
// ListenConfig is a struct to customize startup of Velocity.
type ListenConfig struct {
	// GracefulContext is a field to shutdown Velocity by given context gracefully.
	//
	// Default: nil
	...
	// BeforeServeFunc allows customizing and accessing velocity app before serving the app.
	//
	// Default: nil
	...
	// When set to true, it will not print out the «Velocity» ASCII art and listening address.
	//
	// Default: false
...
func (app *App) startupMessage(addr string, isTLS bool, pids string, cfg ListenConfig) { //nolint: revive // Accepting a bool param named isTLS if fine here
	...
	fmt.Fprintf(out, "%s\n", fmt.Sprintf(figletVelocityText, colors.Red+"v"+Version+colors.Reset)) //nolint:errcheck,revive // ignore error
