...
app.Get("/", func(c velocity.Ctx) error {
    ...
    log.Println(p.Name)     // john
    log.Println(p.Pass)     // doe
    // Depending on velocity.Config{EnableSplittingOnParsers: false} - default
    log.Println(p.Products) // ["shoe,hat"]
    // With velocity.Config{EnableSplittingOnParsers: true}
    // log.Println(p.Products) // ["shoe", "hat"]
...
:::info
For more parser settings, please refer to [Config](velocity.md#enablesplittingonparsers)
:::
...
You can register them using the [RegisterCustomBinder](./app.md#registercustombinder) method of the Velocity instance.
...
func (cb *customBinder) Parse(c velocity.Ctx, out any) error {
    // parse YAML body
...
```go title="Signature"
func SetParserDecoder(parserConfig velocity.ParserConfig{
    IgnoreUnknownKeys bool,
    ParserType        []velocity.ParserType{
        Customtype any,
...
customTime := velocity.ParserType{
    CustomType: CustomTime{},
    Converter:  timeConverter,
}

// Add custom type to the Decoder settings
velocity.SetParserDecoder(velocity.ParserConfig{
    IgnoreUnknownKeys: true,
    ParserType:        []velocity.ParserType{customTime},
    ZeroEmpty:         true,
})
...
app.Post("/body", func(c velocity.Ctx) error {
    var d Demo
...
Specify your struct validator in the [config](./velocity.md#structvalidator).
...
// Setup your validator in the Velocity config
