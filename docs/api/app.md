...
id: app
title: 🚀 App
description: The app instance conventionally denotes the Velocity application.
sidebar_position: 2
---
...
You can mount a Velocity instance using the [`app.Use`](./app.md#use) method, similar to [`Express`](https://expressjs.com/en/api.html#router.use).
...
func main() {
    ...
    micro.Get("/doe", func(c velocity.Ctx) error {
...
func main() {
    ...
    app.Route("/test").Get(func(c velocity.Ctx) error {
        return c.SendString("GET /test")
    })

    app.Route("/events").All(func(c velocity.Ctx) error {
    ...
    app.Route("/v2").Route("/user").Get(func(c velocity.Ctx) error {
        return c.SendString("GET /v2/user")
    })

    // Use multiple methods
    app.Route("/api").Get(func(c velocity.Ctx) error {
...
`Config` returns the [app config](./velocity.md#config) as a value (read-only).
...
func main() {
    ...
    app.NewCtxFunc(func(app *velocity.App) velocity.CustomCtx {
        return &CustomCtx{
            DefaultCtx: *velocity.NewDefaultCtx(app),
        }
    })
...
func (*customBinder) Parse(c velocity.Ctx, out any) error {
    // Parse YAML body
...
func main() {
    ...
    app.Post("/custom", func(c velocity.Ctx) error {
        var user User
    ...
    app.Post("/normal", func(c velocity.Ctx) error {
        var user User
...
func main() {
    ...
    if resp.StatusCode == velocity.StatusOK {
...
func main() {
    ...
    app.Get("/define", func(c velocity.Ctx) error {
        // Define a new route dynamically
        app.Get("/dynamically-defined", func(c velocity.Ctx) error {
            return c.SendStatus(velocity.StatusOK)
        })
