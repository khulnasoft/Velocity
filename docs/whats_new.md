...
[//]: # (https://github.com/khulnasoft/velocity/releases/tag/v3.0.0-beta.2)

## 🎉 Welcome

We are excited to announce the release of Velocity v3! 🚀

In this guide, we'll walk you through the most important changes in Velocity `v3` and show you how to migrate your existing Velocity `v2` applications to Velocity `v3`.

Here's a quick overview of the changes in Velocity `v3`:
...
Velocity `v3` drops support for Go versions below `1.23`. We recommend upgrading to Go `1.23` or higher to use Velocity `v3`.

## 🚀 App

We have made several changes to the Velocity app, including:
...
### Custom Ctx Interface in Velocity v3

Velocity v3 introduces a customizable `Ctx` interface, allowing developers to extend and modify the context to fit their needs. This feature provides greater flexibility and control over request handling.
...
func main() {
    ...
    app.NewCtxFunc(func(app *velocity.App) velocity.Ctx {
        return &CustomCtx{
            Ctx: *velocity.NewCtx(app),
        }
    })
...
```go
app.Listen(":444", velocity.ListenConfig{TLSMinVersion: tls.VersionTLS12})
```
...
app.Route("/api").Route("/user/:id?")
    .Get(func(c velocity.Ctx) error {
        // Get user
        return c.JSON(velocity.Map{"message": "Get user", "id": c.Params("id")})
    })
    .Post(func(c velocity.Ctx) error {
        // Create user
        return c.JSON(velocity.Map{"message": "User created"})
    })
    .Put(func(c velocity.Ctx) error {
        // Update user
        return c.JSON(velocity.Map{"message": "User updated", "id": c.Params("id")})
    })
    .Delete(func(c velocity.Ctx) error {
        // Delete user
        return c.JSON(velocity.Map{"message": "User deleted", "id": c.Params("id")})
    })
```
...
app.Use(["/v1", "/v2"], func(c velocity.Ctx) error {
...
// define subapp
api := velocity.New()
api.Get("/user", func(c velocity.Ctx) error {
...
+    Test(req *http.Request, config ...velocity.TestConfig) (*http.Response, error)
```
...
```go
app.Get("/sse", func(c velocity.Ctx) {
...
The Govelocity client has been completely rebuilt. It includes numerous new features such as Cookiejar, request/response hooks, and more.
You can take a look to [client docs](./client/rest.md) to see what's new with the client.

## 📎 Binding

Velocity v3 introduces a new binding mechanism that simplifies the process of binding request data to structs. The new binding system supports binding from various sources such as URL parameters, query parameters, headers, and request bodies. This unified approach makes it easier to handle different types of request data in a consistent manner.
...
app.Post("/user/:id", func(c velocity.Ctx) error {
    var user User
    if err := c.Bind().Body(&user); err != nil {
        return c.Status(velocity.StatusBadRequest).JSON(velocity.Map{"error": err.Error()})
    }
    return c.JSON(user)
})
```
...
Velocity v3 enhances the redirect functionality by introducing new methods and improving existing ones. The new redirect methods provide more flexibility and control over the redirection process.
...
Velocity v3 introduces new generic functions that provide additional utility and flexibility for developers. These functions are designed to simplify common tasks and improve code readability.
...
func main() {
    ...
    app.Get("/convert", func(c velocity.Ctx) error {
        value, err := velocity.Convert[string](c.Query("value"), strconv.Atoi, 0)
        if err != nil {
...
func main() {
    ...
    app.Get("/user/*", func(c velocity.Ctx) error {
        // get local values
        name := velocity.Locals[string](c, "user")
        age := velocity.Locals[int](c, "age")
        // ...
        return c.JSON(velocity.Map{"name": name, "age": age})
    })
...
func main() {
    ...
    app.Get("/params/:id", func(c velocity.Ctx) error {
        id := velocity.Params[int](c, "id", 0)
        return c.JSON(id)
    })
...
func main() {
    ...
    app.Get("/query", func(c velocity.Ctx) error {
        age := velocity.Query[int](c, "age", 0)
        return c.JSON(age)
    })
...
func main() {
    ...
    app.Get("/header", func(c velocity.Ctx) error {
        userAgent := velocity.GetReqHeader[string](c, "User-Agent", "Unknown")
...
`velocity.AllLogger` interface now has a new method called `Logger`. This method can be used to get the underlying logger instance from the Velocity logger middleware. This is useful when you want to configure the logger middleware with a custom logger and still want to access the underlying logger instance.
...
New helper function called `LoggerToWriter` has been added to the logger middleware. This function allows you to use 3rd party loggers such as `logrus` or `zap` with the Velocity logger middleware without any extra afford. For example, you can use `zap` with Velocity logger middleware like this:
...
Monitor middleware is migrated to the [Contrib package](https://github.com/khulnasoft/contrib/tree/main/monitor) with [PR #1172](https://github.com/khulnasoft/contrib/pull/1172).
...
```go
// Before
app.Add(velocity.MethodPost, "/api", myHandler)
```

```go
// After
app.Add([]string{velocity.MethodPost}, "/api", myHandler)
```
...
app.Route("/api", func(apiGrp Router) {
    apiGrp.Route("/user/:id?", func(userGrp Router) {
        userGrp.Get("/", func(c velocity.Ctx) error {
            // Get user
            return c.JSON(velocity.Map{"message": "Get user", "id": c.Params("id")})
        })
        userGrp.Post("/", func(c velocity.Ctx) error {
            // Create user
            return c.JSON(velocity.Map{"message": "User created"})
        })
    })
...
Velocity v3 introduces several new features and changes to the Ctx interface, enhancing its functionality and flexibility.
...
In Velocity v3, the `Ctx` parameter in handlers is now an interface, which means the `*` symbol is no longer used. Here is an example demonstrating this change:
...
func main() {
    ...
    // Route Handler with *velocity.Ctx
    app.Get("/", func(c *velocity.Ctx) error {
        return c.SendString("Hello, World!")
    })
...
func main() {
    ...
    // Route Handler without *velocity.Ctx
    app.Get("/", func(c velocity.Ctx) error {
        return c.SendString("Hello, World!")
    })
...
In this example, the `Ctx` parameter in the handler is used as an interface (`velocity.Ctx`) instead of a pointer (`*velocity.Ctx`). This change allows for more flexibility and customization in Velocity v3.

</details>

#### 📎 Parser

The `Parser` section in Velocity v3 has undergone significant changes to improve functionality and flexibility.
...
1. **BodyParser**: Use `c.Bind().Body()` instead of `c.BodyParser()`.
    ...
    ```go
    // Before
    app.Post("/user", func(c *velocity.Ctx) error {
        var user User
...
4. **CookieParser**: Use `c.Bind().Cookie()` instead of `c.CookieParser()`.
    ...
    ```go
    // Before
    app.Get("/cookie", func(c *velocity.Ctx) error {
        var cookie Cookie
...
Velocity v3 introduces a completely rebuilt client package with numerous new features such as Cookiejar, request/response hooks, and more. Here is a guide to help you migrate from Velocity v2 to Velocity v3.
...
app.Use(cors.New(cors.Config{
    ...
    AllowMethods: strings.Join([]string{velocity.MethodGet, velocity.MethodPost}, ","),
...
app.Use(cors.New(cors.Config{
    ...
    AllowMethods: []string{velocity.MethodGet, velocity.MethodPost},
...
app.Use(healthcheck.New(healthcheck.Config{
    LivenessProbe: func(c velocity.Ctx) bool {
        return true
    },
    LivenessEndpoint: "/live",
    ReadinessProbe: func(c velocity.Ctx) bool {
...
Since v3 the Monitor middleware has been moved to the [Contrib package](https://github.com/khulnasoft/contrib/tree/main/monitor)

```go
// Before
import "github.com/khulnasoft/velocity/middleware/monitor"
...
```go
// After
import "github.com/khulnasoft/contrib/monitor"
