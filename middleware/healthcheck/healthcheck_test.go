...
func shouldGiveStatus(t *testing.T, app *velocity.App, path string, expectedStatus int) {
	t.Helper()
	req, err := app.Test(httptest.NewRequest(velocity.MethodGet, path, nil))
	require.NoError(t, err)
	require.Equal(t, expectedStatus, req.StatusCode, "path: "+path+" should match "+strconv.Itoa(expectedStatus))
}

func shouldGiveOK(t *testing.T, app *velocity.App, path string) {
	t.Helper()
	shouldGiveStatus(t, app, path, velocity.StatusOK)
}

func shouldGiveNotFound(t *testing.T, app *velocity.App, path string) {
	t.Helper()
	shouldGiveStatus(t, app, path, velocity.StatusNotFound)
}
...
func Test_HealthCheck_Custom(t *testing.T) {
	...
	app.Get("/live", NewHealthChecker(Config{
		Probe: func(_ velocity.Ctx) bool {
			return true
		},
	}))
	...
	req, err := app.Test(httptest.NewRequest(velocity.MethodPost, "/live", nil))
	require.NoError(t, err)
	require.Equal(t, velocity.StatusMethodNotAllowed, req.StatusCode)

	// Ready should return 404 with POST request
	req, err = app.Test(httptest.NewRequest(velocity.MethodPost, "/ready", nil))
	require.NoError(t, err)
	require.Equal(t, velocity.StatusMethodNotAllowed, req.StatusCode)

	// Ready should return 503 with GET request before the channel is closed
	shouldGiveStatus(t, app, "/ready", velocity.StatusServiceUnavailable)

	shouldGiveStatus(t, app, "/startupz", velocity.StatusServiceUnavailable)
...
func Test_HealthCheck_Custom_Nested(t *testing.T) {
	...
	shouldGiveStatus(t, app, "/probe/ready", velocity.StatusServiceUnavailable)
	shouldGiveOK(t, app, "/probe/live/")
	shouldGiveStatus(t, app, "/probe/ready/", velocity.StatusServiceUnavailable)
...
func Benchmark_HealthCheck(b *testing.B) {
	...
	require.Equal(b, velocity.StatusOK, fctx.Response.Header.StatusCode())
}
