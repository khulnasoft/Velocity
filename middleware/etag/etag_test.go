...
func Test_ETag_SkipError(t *testing.T) {
	...
	app.Get("/", func(_ velocity.Ctx) error {
		return velocity.ErrForbidden
	})

	resp, err := app.Test(httptest.NewRequest(velocity.MethodGet, "/", nil))
	require.NoError(t, err)
	require.Equal(t, velocity.StatusForbidden, resp.StatusCode)
}
...
func Test_ETag_NotStatusOK(t *testing.T) {
	...
	app.Get("/", func(c velocity.Ctx) error {
		return c.SendStatus(velocity.StatusCreated)
	})
...
func testETagNewEtag(t *testing.T, headerIfNoneMatch, matched bool) { //nolint:revive // We're in a test, so using bools as a flow-control is fine
	...
	if headerIfNoneMatch {
		...
		req.Header.Set(velocity.HeaderIfNoneMatch, etag)
	}
	...
	if !headerIfNoneMatch || !matched {
		require.Equal(t, velocity.StatusOK, resp.StatusCode)
		require.Equal(t, `"13-1831710635"`, resp.Header.Get(velocity.HeaderETag))
		return
	}

	if matched {
		require.Equal(t, velocity.StatusNotModified, resp.StatusCode)
...
func testETagWeakEtag(t *testing.T, headerIfNoneMatch, matched bool) { //nolint:revive // We're in a test, so using bools as a flow-control is fine
	...
	if !headerIfNoneMatch || !matched {
		require.Equal(t, velocity.StatusOK, resp.StatusCode)
		require.Equal(t, `W/"13-1831710635"`, resp.Header.Get(velocity.HeaderETag))
		return
	}
...
func testETagCustomEtag(t *testing.T, headerIfNoneMatch, matched bool) { //nolint:revive // We're in a test, so using bools as a flow-control is fine
	...
	app.Get("/", func(c velocity.Ctx) error {
		c.Set(velocity.HeaderETag, `"custom"`)
		if bytes.Equal(c.Request().Header.Peek(velocity.HeaderIfNoneMatch), []byte(`"custom"`)) {
			return c.SendStatus(velocity.StatusNotModified)
		}
		return c.SendString("Hello, World!")
	})
	...
	if !headerIfNoneMatch || !matched {
		require.Equal(t, velocity.StatusOK, resp.StatusCode)
		require.Equal(t, `"custom"`, resp.Header.Get(velocity.HeaderETag))
		return
	}
...
func Test_ETag_CustomEtagPut(t *testing.T) {
	...
	app.Put("/", func(c velocity.Ctx) error {
		c.Set(velocity.HeaderETag, `"custom"`)
		if !bytes.Equal(c.Request().Header.Peek(velocity.HeaderIfMatch), []byte(`"custom"`)) {
			return c.SendStatus(velocity.StatusPreconditionFailed)
		}
		return c.SendString("Hello, World!")
	})

	req := httptest.NewRequest(velocity.MethodPut, "/", nil)
	req.Header.Set(velocity.HeaderIfMatch, `"non-match"`)
	resp, err := app.Test(req)
	require.NoError(t, err)
	require.Equal(t, velocity.StatusPreconditionFailed, resp.StatusCode)
}
...
func Benchmark_Etag(b *testing.B) {
	...
	require.Equal(b, `"13-1831710635"`, string(fctx.Response.Header.Peek(velocity.HeaderETag)))
}
