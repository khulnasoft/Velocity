...
import (
	...
	clientpkg "go.khulnasoft.com/velocity/v3/client"
...
func startServer(app *velocity.App, ln net.Listener) {
	go func() {
		err := app.Listener(ln, velocity.ListenConfig{
...
func createProxyTestServer(t *testing.T, handler velocity.Handler, network, address string) (*velocity.App, string) {
	t.Helper()
...
func createProxyTestServerIPv4(t *testing.T, handler velocity.Handler) (*velocity.App, string) {
	t.Helper()
	return createProxyTestServer(t, handler, velocity.NetworkTCP4, "127.0.0.1:0")
}

func createProxyTestServerIPv6(t *testing.T, handler velocity.Handler) (*velocity.App, string) {
	t.Helper()
	return createProxyTestServer(t, handler, velocity.NetworkTCP6, "[::1]:0")
}
...
func Test_Proxy(t *testing.T) {
	...
	target, addr := createProxyTestServerIPv4(t, func(c velocity.Ctx) error {
		return c.SendStatus(velocity.StatusTeapot)
	})

	resp, err := target.Test(httptest.NewRequest(velocity.MethodGet, "/", nil), velocity.TestConfig{
...
func Test_Proxy_Balancer_WithTlsConfig(t *testing.T) {
	...
	ln, err := net.Listen(velocity.NetworkTCP4, "127.0.0.1:0")
	require.NoError(t, err)
	...
	app.Get("/tlsbalancer", func(c velocity.Ctx) error {
...
func Test_Proxy_Balancer_IPv6_Upstream(t *testing.T) {
	...
	target, addr := createProxyTestServerIPv6(t, func(c velocity.Ctx) error {
		return c.SendStatus(velocity.StatusTeapot)
	})
...
func Test_Proxy_Forward_WithTlsConfig_To_Http(t *testing.T) {
	...
	_, targetAddr := createProxyTestServerIPv4(t, func(c velocity.Ctx) error {
	...
	proxyServerLn, err := net.Listen(velocity.NetworkTCP4, "127.0.0.1:0")
	require.NoError(t, err)
...
func Test_Proxy_Forward(t *testing.T) {
	...
	_, addr := createProxyTestServerIPv4(t, func(c velocity.Ctx) error {
...
func Test_Proxy_Forward_WithClient_TLSConfig(t *testing.T) {
	...
	app.Get("/tlsfwd", func(c velocity.Ctx) error {
...
func Test_Proxy_Modify_Response(t *testing.T) {
	...
	app.Use(Balancer(Config{
		...
		ModifyResponse: func(c velocity.Ctx) error {
			c.Response().SetStatusCode(velocity.StatusOK)
...
func Test_Proxy_Timeout_Slow_Server(t *testing.T) {
	...
	_, addr := createProxyTestServerIPv4(t, func(c velocity.Ctx) error {
		...
		return c.SendString("velocity is awesome")
	})
	...
	resp, err := app.Test(httptest.NewRequest(velocity.MethodGet, "/", nil), velocity.TestConfig{
		Timeout:       2 * time.Second,
		FailOnTimeout: true,
	})
	require.NoError(t, err)
	...
	b, err := io.ReadAll(resp.Body)
	require.NoError(t, err)
	require.Equal(t, "velocity is awesome", string(b))
}
...
func Test_Proxy_Do_RestoreOriginalURL(t *testing.T) {
	...
	resp, err1 := app.Test(httptest.NewRequest(velocity.MethodGet, "/test", nil))
...
func Test_Proxy_DoRedirects_RestoreOriginalURL(t *testing.T) {
	...
	resp, err1 := app.Test(httptest.NewRequest(velocity.MethodGet, "/test", nil), velocity.TestConfig{
		Timeout:       2 * time.Second,
		FailOnTimeout: true,
	})
	require.NoError(t, err1)
...
func Test_Proxy_DoDeadline_PastDeadline(t *testing.T) {
	...
	_, err1 := app.Test(httptest.NewRequest(velocity.MethodGet, "/test", nil), velocity.TestConfig{
...
func Test_Proxy_Do_HTTP_Prefix_URL(t *testing.T) {
	...
	resp, err := app.Test(httptest.NewRequest(velocity.MethodGet, "/http://"+addr, nil))
	require.NoError(t, err)
...
func Test_Proxy_Forward_Global_Client(t *testing.T) {
	...
	app := velocity.New()
	app.Get("/test_global_client", func(c velocity.Ctx) error {
...
func Test_Proxy_Forward_Local_Client(t *testing.T) {
	...
	require.NoError(t, err)
	app := velocity.New()
	app.Get("/test_local_client", func(c velocity.Ctx) error {
...
func Test_Proxy_Domain_Forward_Local(t *testing.T) {
	...
	// target server
	ln1, err := net.Listen(velocity.NetworkTCP4, "127.0.0.1:0")
	require.NoError(t, err)
	app1 := velocity.New()

	app1.Get("/test", func(c velocity.Ctx) error {
