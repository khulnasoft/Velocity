...
func New(config ...Config) velocity.Handler {
	...
	return func(c velocity.Ctx) error {
		...
		switch c.Method() {
		case velocity.MethodGet, velocity.MethodHead, velocity.MethodOptions, velocity.MethodTrace:
...
func getRawFromStorage(c velocity.Ctx, token string, cfg Config, sessionManager *sessionManager, storageManager *storageManager) []byte {
	if cfg.Session != nil {
...
func createOrExtendTokenInStorage(c velocity.Ctx, token string, cfg Config, sessionManager *sessionManager, storageManager *storageManager) {
	if cfg.Session != nil {
...
func deleteTokenFromStorage(c velocity.Ctx, token string, cfg Config, sessionManager *sessionManager, storageManager *storageManager) {
	if cfg.Session != nil {
...
func updateCSRFCookie(c velocity.Ctx, cfg Config, token string) {
	setCSRFCookie(c, cfg, token, cfg.IdleTimeout)
}

func expireCSRFCookie(c velocity.Ctx, cfg Config) {
	setCSRFCookie(c, cfg, "", -time.Hour)
}

func setCSRFCookie(c velocity.Ctx, cfg Config, token string, expiry time.Duration) {
	cookie := &velocity.Cookie{
		Name:        cfg.CookieName,
		Value:       token,
...
func (handler *Handler) DeleteToken(c velocity.Ctx) error {
...
func originMatchesHost(c velocity.Ctx, trustedOrigins []string, trustedSubOrigins []subdomain) error {
	origin := strings.ToLower(c.Get(velocity.HeaderOrigin))
...
func refererMatchesHost(c velocity.Ctx, trustedOrigins []string, trustedSubOrigins []subdomain) error {
	referer := strings.ToLower(c.Get(velocity.HeaderReferer))
	if referer == "" {
