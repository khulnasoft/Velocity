...
func (m *sessionManager) getRaw(c velocity.Ctx, key string, raw []byte) []byte {
...
func (m *sessionManager) setRaw(c velocity.Ctx, key string, raw []byte, exp time.Duration) {
	sess := session.FromContext(c)
	if sess != nil {
...
func (m *sessionManager) delRaw(c velocity.Ctx) {
	sess := session.FromContext(c)
	if sess != nil {
