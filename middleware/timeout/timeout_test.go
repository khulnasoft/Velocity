...
func TestTimeout_Success(t *testing.T) {
	...
	app.Get("/fast", New(func(c velocity.Ctx) error {
		// Simulate some work
	...
	req := httptest.NewRequest(velocity.MethodGet, "/fast", nil)
	resp, err := app.Test(req)
	require.NoError(t, err, "app.Test(req) should not fail")
	require.Equal(t, velocity.StatusOK, resp.StatusCode, "Expected 200 OK for fast requests")
}
...
func TestTimeout_Exceeded(t *testing.T) {
	...
	app.Get("/slow", New(func(c velocity.Ctx) error {
	...
	req := httptest.NewRequest(velocity.MethodGet, "/slow", nil)
	resp, err := app.Test(req)
	require.NoError(t, err, "app.Test(req) should not fail")
	require.Equal(t, velocity.StatusRequestTimeout, resp.StatusCode, "Expected 408 Request Timeout")
}
...
func TestTimeout_CustomError(t *testing.T) {
	...
	app.Get("/custom", New(func(c velocity.Ctx) error {
	...
	req := httptest.NewRequest(velocity.MethodGet, "/custom", nil)
	resp, err := app.Test(req)
	require.NoError(t, err, "app.Test(req) should not fail")
	require.Equal(t, velocity.StatusRequestTimeout, resp.StatusCode, "Expected 408 for custom timeout error")
}
...
func TestTimeout_UnmatchedError(t *testing.T) {
	...
	app.Get("/unmatched", New(func(_ velocity.Ctx) error {
		return errUnrelated // Not in the custom error list
	}, 100*time.Millisecond, errCustomTimeout))

	req := httptest.NewRequest(velocity.MethodGet, "/unmatched", nil)
	resp, err := app.Test(req)
	require.NoError(t, err, "app.Test(req) should not fail")
...
func TestTimeout_ZeroDuration(t *testing.T) {
	...
	app.Get("/zero", New(func(c velocity.Ctx) error {
		// Sleep 50ms, but there's no real 'deadline' since zero-timeout.
		time.Sleep(50 * time.Millisecond)
		return c.SendString("No timeout used")
	}, 0))

	req := httptest.NewRequest(velocity.MethodGet, "/zero", nil)
	resp, err := app.Test(req)
	require.NoError(t, err, "app.Test(req) should not fail")
	require.Equal(t, velocity.StatusOK, resp.StatusCode, "Expected 200 OK with zero timeout")
}
