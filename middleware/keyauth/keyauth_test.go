	...
	for _, authSource := range testSources {
		t.Run(authSource, func(t *testing.T) {
			for _, test := range tests {
				...
				app.Get(route, func(c velocity.Ctx) error {
					return c.SendString("Success!")
				})

				// construct the test HTTP request
				var req *http.Request
				req, err := http.NewRequestWithContext(context.Background(), velocity.MethodGet, test.route, nil)
...
func TestPanicOnInvalidConfiguration(t *testing.T) {
	...
	require.Panics(t, func() {
		authMiddleware := New(Config{
			KeyLookup: "invalid",
			Validator: func(_ velocity.Ctx, _ string) (bool, error) {
				return true, nil
			},
		})
		// We shouldn't even make it this far, but these next two lines prevent authMiddleware from being an unused variable.
...
func TestMultipleKeyLookup(t *testing.T) {
	...
	// construct the test HTTP request
	var req *http.Request
	req, err = http.NewRequestWithContext(context.Background(), velocity.MethodGet, "/foo", nil)
	require.NoError(t, err)
	q := req.URL.Query()
...
func Test_MultipleKeyAuth(t *testing.T) {
	...
	app.Get("/auth1", func(c velocity.Ctx) error {
...
func Test_CustomSuccessAndFailureHandlers(t *testing.T) {
	...
	app.Use(New(Config{
		SuccessHandler: func(c velocity.Ctx) error {
			return c.Status(velocity.StatusOK).SendString("API key is valid and request was handled by custom success handler")
		},
		ErrorHandler: func(c velocity.Ctx, _ error) error {
			return c.Status(velocity.StatusUnauthorized).SendString("API key is invalid and request was handled by custom error handler")
		},
		Validator: func(_ velocity.Ctx, key string) (bool, error) {
			if key == CorrectKey {
				return true, nil
	...
	res, err := app.Test(httptest.NewRequest(velocity.MethodGet, "/", nil))
	require.NoError(t, err)
...
func Test_CustomNextFunc(t *testing.T) {
	...
	app.Get("/allowed", func(c velocity.Ctx) error {
	...
	req := httptest.NewRequest(velocity.MethodGet, "/allowed", nil)
	res, err := app.Test(req)
	require.NoError(t, err)
	...
	req = httptest.NewRequest(velocity.MethodGet, "/not-allowed", nil)
	res, err = app.Test(req)
	require.NoError(t, err)
