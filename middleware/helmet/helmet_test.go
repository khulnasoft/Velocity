...
func Test_Default(t *testing.T) {
	...
	require.Equal(t, "0", resp.Header.Get(velocity.HeaderXXSSProtection))
	require.Equal(t, "nosniff", resp.Header.Get(velocity.HeaderXContentTypeOptions))
	require.Equal(t, "SAMEORIGIN", resp.Header.Get(velocity.HeaderXFrameOptions))
	require.Equal(t, "", resp.Header.Get(velocity.HeaderContentSecurityPolicy))
	require.Equal(t, "no-referrer", resp.Header.Get(velocity.HeaderReferrerPolicy))
	require.Equal(t, "", resp.Header.Get(velocity.HeaderPermissionsPolicy))
...
func Test_CustomValues_AllHeaders(t *testing.T) {
	...
	require.Equal(t, "0", resp.Header.Get(velocity.HeaderXXSSProtection))
	require.Equal(t, "custom-nosniff", resp.Header.Get(velocity.HeaderXContentTypeOptions))
	require.Equal(t, "DENY", resp.Header.Get(velocity.HeaderXFrameOptions))
	require.Equal(t, "default-src 'none'", resp.Header.Get(velocity.HeaderContentSecurityPolicyReportOnly))
	require.Equal(t, "origin", resp.Header.Get(velocity.HeaderReferrerPolicy))
	require.Equal(t, "geolocation=(self)", resp.Header.Get(velocity.HeaderPermissionsPolicy))
...
func Test_RealWorldValues_AllHeaders(t *testing.T) {
	...
	require.Equal(t, "nosniff", resp.Header.Get(velocity.HeaderXContentTypeOptions))
	require.Equal(t, "SAMEORIGIN", resp.Header.Get(velocity.HeaderXFrameOptions))
	require.Equal(t, "default-src 'self';base-uri 'self';font-src 'self' https: data:;form-action 'self';frame-ancestors 'self';img-src 'self' data:;object-src 'none';script-src 'self';script-src-attr 'none';style-src 'self' https: 'unsafe-inline';upgrade-insecure-requests", resp.Header.Get(velocity.HeaderContentSecurityPolicy))
	require.Equal(t, "no-referrer", resp.Header.Get(velocity.HeaderReferrerPolicy))
	require.Equal(t, "geolocation=(self)", resp.Header.Get(velocity.HeaderPermissionsPolicy))
	require.Equal(t, "require-corp", resp.Header.Get("Cross-Origin-Embedder-Policy"))
	require.Equal(t, "same-origin", resp.Header.Get("Cross-Origin-Opener-Policy"))
...
func Test_Next(t *testing.T) {
	...
	app.Use(New(Config{
		Next: func(ctx velocity.Ctx) bool {
	...
	app.Get("/next", func(c velocity.Ctx) error {
	...
	resp, err = app.Test(httptest.NewRequest(velocity.MethodGet, "/next", nil))
	require.NoError(t, err)
	require.Equal(t, "", resp.Header.Get(velocity.HeaderReferrerPolicy))
}
...
func Test_ContentSecurityPolicy(t *testing.T) {
	...
	resp, err := app.Test(httptest.NewRequest(velocity.MethodGet, "/", nil))
	require.NoError(t, err)
	require.Equal(t, "default-src 'none'", resp.Header.Get(velocity.HeaderContentSecurityPolicy))
}
...
func Test_PermissionsPolicy(t *testing.T) {
	...
	resp, err := app.Test(httptest.NewRequest(velocity.MethodGet, "/", nil))
	require.NoError(t, err)
	require.Equal(t, "microphone=()", resp.Header.Get(velocity.HeaderPermissionsPolicy))
}
