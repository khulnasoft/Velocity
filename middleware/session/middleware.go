// Package session provides session management middleware for Velocity.
...
//
// Returns:
//   - velocity.Handler: The Velocity handler for the session middleware.
//
// Usage:
//
//	app.Use(session.New())
...
func (m *Middleware) initialize(c velocity.Ctx, cfg Config) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
// FromContext returns the Middleware from the Velocity context.
//
// Parameters:
//   - c: The Velocity context.
//
// Returns:
