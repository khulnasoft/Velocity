// Special thanks to @codemicro for moving this to velocity core
// Original middleware: github.com/codemicro/velocity-cache
package cache
...
func Test_Cache_CacheControl(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, "public, max-age=10", resp.Header.Get(velocity.HeaderCacheControl))
}
...
func Test_Cache_Expired(t *testing.T) {
	...
	respCached, err := app.Test(httptest.NewRequest(velocity.MethodGet, "/", nil))
	require.NoError(t, err)
	...
	respCachedNextRound, err := app.Test(httptest.NewRequest(velocity.MethodGet, "/", nil))
	require.NoError(t, err)
...
func Test_Cache(t *testing.T) {
	...
	cachedReq := httptest.NewRequest(velocity.MethodGet, "/", nil)
...
func Test_Cache_WithNoCacheRequestDirective(t *testing.T) {
	...
	app.Get("/", func(c velocity.Ctx) error {
		return c.SendString(velocity.Query(c, "id", "1"))
	})
	...
	cachedReq := httptest.NewRequest(velocity.MethodGet, "/?id=2", nil)
	cachedResp, err := app.Test(cachedReq)
	require.NoError(t, err)
	cachedBody, err := io.ReadAll(cachedResp.Body)
	require.NoError(t, err)
	...
	noCacheReq := httptest.NewRequest(velocity.MethodGet, "/?id=2", nil)
	noCacheReq.Header.Set(velocity.HeaderCacheControl, noCache)
	...
	noCacheReq1 := httptest.NewRequest(velocity.MethodGet, "/?id=2", nil)
	noCacheReq1.Header.Set(velocity.HeaderCacheControl, noCache)
	...
	cachedReq1 := httptest.NewRequest(velocity.MethodGet, "/", nil)
...
func Test_Cache_WithETagAndNoCacheRequestDirective(t *testing.T) {
	...
	cachedReq := httptest.NewRequest(velocity.MethodGet, "/?id=2", nil)
	cachedReq.Header.Set(velocity.HeaderIfNoneMatch, etagToken)
	cachedResp, err := app.Test(cachedReq)
	require.NoError(t, err)
	require.Equal(t, cacheHit, cachedResp.Header.Get("X-Cache"))
	require.Equal(t, velocity.StatusNotModified, cachedResp.StatusCode)
	...
	noCacheReq := httptest.NewRequest(velocity.MethodGet, "/?id=2", nil)
	noCacheReq.Header.Set(velocity.HeaderCacheControl, noCache)
	noCacheReq.Header.Set(velocity.HeaderIfNoneMatch, etagToken)
	noCacheResp, err := app.Test(noCacheReq)
	require.NoError(t, err)
	require.Equal(t, cacheMiss, noCacheResp.Header.Get("X-Cache"))
	require.Equal(t, velocity.StatusOK, noCacheResp.StatusCode)
	...
	noCacheReq1 := httptest.NewRequest(velocity.MethodGet, "/?id=2", nil)
	noCacheReq1.Header.Set(velocity.HeaderCacheControl, noCache)
	noCacheReq1.Header.Set(velocity.HeaderIfNoneMatch, etagToken)
	noCacheResp1, err := app.Test(noCacheReq1)
	require.NoError(t, err)
	require.Equal(t, cacheMiss, noCacheResp1.Header.Get("X-Cache"))
	require.Equal(t, velocity.StatusNotModified, noCacheResp1.StatusCode)
	...
	require.NoError(t, err)
	require.Equal(t, cacheHit, cachedResp1.Header.Get("X-Cache"))
	require.Equal(t, velocity.StatusOK, cachedResp1.StatusCode)
	// Response not cached, returns cached response, entry id = 2
}
...
func Test_Cache_WithNoStoreRequestDirective(t *testing.T) {
	...
	// Request id = 2
	noStoreReq := httptest.NewRequest(velocity.MethodGet, "/?id=2", nil)
	noStoreReq.Header.Set(velocity.HeaderCacheControl, noStore)
...
func Test_Cache_WithSeveralRequests(t *testing.T) {
	...
	for runs := 0; runs < 10; runs++ {
		for i := 0; i < 10; i++ {
			func(id int) {
				rsp, err := app.Test(httptest.NewRequest(velocity.MethodGet, fmt.Sprintf("/%d", id), nil))
				require.NoError(t, err)
...
func Test_Cache_Get(t *testing.T) {
	...
	app.Post("/", func(c velocity.Ctx) error {
		return c.SendString(velocity.Query[string](c, "cache"))
	})
	...
	resp, err := app.Test(httptest.NewRequest(velocity.MethodPost, "/?cache=123", nil))
	require.NoError(t, err)
	body, err := io.ReadAll(resp.Body)
	require.NoError(t, err)
	require.Equal(t, "123", string(body))

	resp, err = app.Test(httptest.NewRequest(velocity.MethodPost, "/?cache=12345", nil))
	require.NoError(t, err)
	body, err = io.ReadAll(resp.Body)
	require.NoError(t, err)
	require.Equal(t, "12345", string(body))

	resp, err = app.Test(httptest.NewRequest(velocity.MethodGet, "/get?cache=123", nil))
	require.NoError(t, err)
	body, err = io.ReadAll(resp.Body)
	require.NoError(t, err)
	require.Equal(t, "123", string(body))

	resp, err = app.Test(httptest.NewRequest(velocity.MethodGet, "/get?cache=12345", nil))
	require.NoError(t, err)
	body, err = io.ReadAll(resp.Body)
	require.NoError(t, err)
	require.Equal(t, "123", string(body))
...
func Test_Cache_Post(t *testing.T) {
	...
	app.Use(New(Config{
		Methods: []string{velocity.MethodPost},
	}))
...
func Test_Cache_CustomNext(t *testing.T) {
	...
	app.Use(New(Config{
		Next: func(c velocity.Ctx) bool {
			return c.Response().StatusCode() != velocity.StatusOK
		},
		CacheControl: true,
	}))
	...
	errorCount := 0
	app.Get("/error", func(c velocity.Ctx) error {
		errorCount++
		return c.Status(velocity.StatusInternalServerError).SendString(strconv.Itoa(errorCount))
	})
	...
	require.NotEmpty(t, respCached.Header.Get(velocity.HeaderCacheControl))

	_, err = app.Test(httptest.NewRequest(velocity.MethodGet, "/error", nil))
	require.NoError(t, err)

	errRespCached, err := app.Test(httptest.NewRequest(velocity.MethodGet, "/error", nil))
	require.NoError(t, err)
	require.Empty(t, errRespCached.Header.Get(velocity.HeaderCacheControl))
}
...
func Test_CustomKey(t *testing.T) {
	...
	app := velocity.New()
	var called bool
	app.Use(New(Config{KeyGenerator: func(c velocity.Ctx) string {
		called = true
...
func Test_CustomExpiration(t *testing.T) {
	...
	app.Use(New(Config{ExpirationGenerator: func(c velocity.Ctx, _ *Config) time.Duration {
		called = true
		var err error
	...
	cachedResp, err := app.Test(httptest.NewRequest(velocity.MethodGet, "/", nil))
	require.NoError(t, err)
	...
	cachedRespNextRound, err := app.Test(httptest.NewRequest(velocity.MethodGet, "/", nil))
	require.NoError(t, err)
...
func Test_CacheHeader(t *testing.T) {
	...
	app.Get("/error", func(c velocity.Ctx) error {
		count++
		c.Response().Header.Add("Cache-Time", "1")
		return c.Status(velocity.StatusInternalServerError).SendString(strconv.Itoa(count))
	})
...
func Test_Cache_WithHead(t *testing.T) {
	...
	req := httptest.NewRequest(velocity.MethodHead, "/", nil)
	resp, err := app.Test(req)
	require.NoError(t, err)
	require.Equal(t, cacheMiss, resp.Header.Get("X-Cache"))

	cachedReq := httptest.NewRequest(velocity.MethodHead, "/", nil)
	cachedResp, err := app.Test(cachedReq)
	require.NoError(t, err)
	require.Equal(t, cacheHit, cachedResp.Header.Get("X-Cache"))
...
func Test_Cache_WithHeadThenGet(t *testing.T) {
	...
	headResp, err := app.Test(httptest.NewRequest(velocity.MethodHead, "/?cache=123", nil))
	require.NoError(t, err)
	...
	getResp, err := app.Test(httptest.NewRequest(velocity.MethodGet, "/?cache=123", nil))
	require.NoError(t, err)
...
func Test_CacheInvalidation(t *testing.T) {
	...
	app.Use(New(Config{
		CacheControl: true,
		CacheInvalidator: func(c velocity.Ctx) bool {
			return velocity.Query[bool](c, "invalidate")
		},
	}))
	...
	respInvalidate, err := app.Test(httptest.NewRequest(velocity.MethodGet, "/?invalidate=true", nil))
	require.NoError(t, err)
...
func Test_CacheInvalidation_noCacheEntry(t *testing.T) {
	...
	t.Run("Cache Invalidator should not be called if no cache entry exist ", func(t *testing.T) {
		...
		_, err := app.Test(httptest.NewRequest(velocity.MethodGet, "/?invalidate=true", nil))
		require.NoError(t, err)
...
func stableAscendingExpiration() func(c1 velocity.Ctx, c2 *Config) time.Duration {
	i := 0
	return func(_ velocity.Ctx, _ *Config) time.Duration {
		i++
...
func Test_Cache_MaxBytesOrder(t *testing.T) {
	...
	for idx, tcase := range cases {
		rsp, err := app.Test(httptest.NewRequest(velocity.MethodGet, tcase[0], nil))
		require.NoError(t, err)
...
func Test_Cache_UncacheableStatusCodes(t *testing.T) {
	...
	app.Get("/:statusCode", func(c velocity.Ctx) error {
	...
	uncacheableStatusCodes := []int{
		...
		velocity.StatusContinue,
		velocity.StatusSwitchingProtocols,
		velocity.StatusProcessing,
		velocity.StatusEarlyHints,

		// Successful responses
		velocity.StatusCreated,
		velocity.StatusAccepted,
		velocity.StatusResetContent,
		velocity.StatusMultiStatus,
		velocity.StatusAlreadyReported,
		velocity.StatusIMUsed,

		// Redirection responses
		velocity.StatusFound,
		velocity.StatusSeeOther,
		velocity.StatusNotModified,
		velocity.StatusUseProxy,
		velocity.StatusSwitchProxy,
		velocity.StatusTemporaryRedirect,
		velocity.StatusPermanentRedirect,

		// Client error responses
		velocity.StatusBadRequest,
		velocity.StatusUnauthorized,
		velocity.StatusPaymentRequired,
		velocity.StatusForbidden,
		velocity.StatusNotAcceptable,
		velocity.StatusProxyAuthRequired,
		velocity.StatusRequestTimeout,
		velocity.StatusConflict,
		velocity.StatusLengthRequired,
		velocity.StatusPreconditionFailed,
		velocity.StatusRequestEntityTooLarge,
		velocity.StatusUnsupportedMediaType,
		velocity.StatusRequestedRangeNotSatisfiable,
		velocity.StatusExpectationFailed,
		velocity.StatusMisdirectedRequest,
		velocity.StatusUnprocessableEntity,
		velocity.StatusLocked,
		velocity.StatusFailedDependency,
		velocity.StatusTooEarly,
		velocity.StatusUpgradeRequired,
		velocity.StatusPreconditionRequired,
		velocity.StatusTooManyRequests,
		velocity.StatusRequestHeaderFieldsTooLarge,
		velocity.StatusUnavailableForLegalReasons,

		// Server error responses
		velocity.StatusInternalServerError,
		velocity.StatusBadGateway,
		velocity.StatusServiceUnavailable,
		velocity.StatusGatewayTimeout,
		velocity.StatusHTTPVersionNotSupported,
		velocity.StatusVariantAlsoNegotiates,
		velocity.StatusInsufficientStorage,
		velocity.StatusLoopDetected,
		velocity.StatusNotExtended,
		velocity.StatusNetworkAuthenticationRequired,
	}
	for _, v := range uncacheableStatusCodes {
		resp, err := app.Test(httptest.NewRequest(velocity.MethodGet, fmt.Sprintf("/%d", v), nil))
		require.NoError(t, err)
		require.Equal(t, cacheUnreachable, resp.Header.Get("X-Cache"))
...
func Benchmark_Cache(b *testing.B) {
	...
	app.Get("/demo", func(c velocity.Ctx) error {
		data, _ := os.ReadFile("../../.github/README.md") //nolint:errcheck // We're inside a benchmark
		return c.Status(velocity.StatusTeapot).Send(data)
	})
...
func Benchmark_Cache_MaxSize(b *testing.B) {
	...
	for i, size := range cases {
		b.Run(names[i], func(b *testing.B) {
			...
			app.Get("/*", func(c velocity.Ctx) error {
				return c.Status(velocity.StatusTeapot).SendString("1")
			})
