...
var cacheableStatusCodes = map[int]bool{
	velocity.StatusOK:                          true,
	velocity.StatusNonAuthoritativeInformation: true,
	velocity.StatusNoContent:                   true,
	velocity.StatusPartialContent:              true,
	velocity.StatusMultipleChoices:             true,
	velocity.StatusMovedPermanently:            true,
	velocity.StatusNotFound:                    true,
	velocity.StatusMethodNotAllowed:            true,
	velocity.StatusGone:                        true,
	velocity.StatusRequestURITooLong:           true,
	velocity.StatusTeapot:                      true,
	velocity.StatusNotImplemented:              true,
}
...
func New(config ...Config) velocity.Handler {
	...
	return func(c velocity.Ctx) error {
		...
		if e != nil {
			...
			if e.exp != 0 && ts >= e.exp {
				...
				if len(e.cencoding) > 0 {
					c.Response().Header.SetBytesV(velocity.HeaderContentEncoding, e.cencoding)
				}
				...
				if cfg.CacheControl {
					...
					c.Set(velocity.HeaderCacheControl, "public, max-age="+maxAge)
				}
		...
		// Continue stack, return err to Velocity if exist
		...
		e.cencoding = utils.CopyBytes(c.Response().Header.Peek(velocity.HeaderContentEncoding))
...
func hasRequestDirective(c velocity.Ctx, directive string) bool {
	return strings.Contains(c.Get(velocity.HeaderCacheControl), directive)
}
